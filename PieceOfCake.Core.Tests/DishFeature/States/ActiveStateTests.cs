...
public class ActiveStateTests : TestsBase
{
    private readonly ActiveState _activeState;

    public ActiveStateTests ()
    {
        _activeState = new ActiveState(Resources);
    }

    [Test]
    public void ActiveState_To_Active_Is_Allowed()
    {
    ...
    [Test]
    public void ActiveState_To_Active_Should_Fail_If_Callback_Is_Error ()
    {
        var callbackMock = new Mock<Func<Result>>();
    ...
    [Test]
    public void ActiveState_To_AwaitingApproval_Is_Forbidden()
    {
        var callbackMock = new Mock<Func<Result>>();
        callbackMock.Setup(x => x.Invoke()).Returns(Result.Success());
    ...
    [Test]
    public void ActiveState_To_Draft_Is_Allowed ()
    {
        var callbackMock = new Mock<Func<Result>>();
        callbackMock.Setup(x => x.Invoke()).Returns(Result.Success());
    ...
    [Test]
    public void ActiveState_To_Rejected_Is_Allowed ()
    {
        var callbackMock = new Mock<Func<Result>>();
        callbackMock.Setup(x => x.Invoke()).Returns(Result.Success());
